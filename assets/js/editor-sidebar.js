...
(function (wp) {
	...
	const NoindexSeoSidebarPanel = function () {
		...
		const handleOverrideChange = function (value) {
			...
			if (!value) {
				updateMeta('_noindex_seo_noindex', false);
				updateMeta('_noindex_seo_nofollow', false);
				updateMeta('_noindex_seo_noarchive', false);
				updateMeta('_noindex_seo_nosnippet', false);
				updateMeta('_noindex_seo_noimageindex', false);
			}
		};
		...
		if (override) {
			if (noindex) activeDirectives.push('noindex');
			if (nofollow) activeDirectives.push('nofollow');
			if (noarchive) activeDirectives.push('noarchive');
			if (nosnippet) activeDirectives.push('nosnippet');
			if (noimageindex) activeDirectives.push('noimageindex');
		}
		...
		return el(
			...
			[
				...
				override && el('div', {
					...
					el(PanelRow, { key: 'noindex-row' },
						el(CheckboxControl, {
							label: '🔍 ' + __('noindex', 'noindex-seo'),
							help: __('Prevent search engines from indexing', 'noindex-seo'),
							checked: noindex,
							onChange: function (value) { updateMeta('_noindex_seo_noindex', value); },
						})
					),

					el(PanelRow, { key: 'nofollow-row' },
						el(CheckboxControl, {
							label: '🔗 ' + __('nofollow', 'noindex-seo'),
							help: __('Prevent search engines from following links', 'noindex-seo'),
							checked: nofollow,
							onChange: function (value) { updateMeta('_noindex_seo_nofollow', value); },
						})
					),

					el(PanelRow, { key: 'noarchive-row' },
						el(CheckboxControl, {
							label: '💾 ' + __('noarchive', 'noindex-seo'),
							help: __('Prevent cached versions in search results', 'noindex-seo'),
							checked: noarchive,
							onChange: function (value) { updateMeta('_noindex_seo_noarchive', value); },
						})
					),

					el(PanelRow, { key: 'nosnippet-row' },
						el(CheckboxControl, {
							label: '📄 ' + __('nosnippet', 'noindex-seo'),
							help: __('Prevent text snippets in search results', 'noindex-seo'),
							checked: nosnippet,
							onChange: function (value) { updateMeta('_noindex_seo_nosnippet', value); },
						})
					),

					el(PanelRow, { key: 'noimageindex-row' },
						el(CheckboxControl, {
							label: '🖼️ ' + __('noimageindex', 'noindex-seo'),
							help: __('Prevent image indexing', 'noindex-seo'),
							checked: noimageindex,
							onChange: function (value) { updateMeta('_noindex_seo_noimageindex', value); },
						})
					),

					// Preview section
					activeDirectives.length > 0 && el('div', {
						key: 'preview-section',
						style: {
							marginTop: '16px',
							padding: '12px',
	...
	// Register the plugin
	registerPlugin('noindex-seo-sidebar', {
