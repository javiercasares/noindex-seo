...
(function($) {
	 ...
	 */
	function initTabs() {
		$('.noindex-seo-tab').on('click', function(e) {
			e.preventDefault();
	...
	function initCollapsibleCards() {
		$('.noindex-seo-card-header').on('click', function() {
		...
		Object.keys(collapsedCards).forEach(function(cardId) {
	 ...
	 */
	function initSearch() {
		$('.noindex-seo-search input').on('input', function() {
			const searchTerm = $(this).val().toLowerCase();

			$('.noindex-seo-option').each(function() {
			...
			$('.noindex-seo-card').each(function() {
	...
	function showSuccessMessage(message) {
		...
		setTimeout(function() {
			$('.noindex-seo-success').fadeOut(function() {
				$(this).remove();
			});
		}, 5000);
	}
